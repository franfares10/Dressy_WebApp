...
PORTS = [8080,8200,8085,9100,8082]

BUFFER_SIZE = 65536
HOST = "127.0.0.1"

#Script que itera los puertos que usa Dressy para ver si están abiertos o no.
#En caso de que alguno este abierto
...
for port in PORTS:
    with closing(testingSocket) as sock:
        if testingSocket.connect_ex((HOST, port)) == 0:
            print(f"Puerto con la dirección {port} esta abierto")
        else:
            print(f"Puerto con la dirección {port} NO esta abierto")
